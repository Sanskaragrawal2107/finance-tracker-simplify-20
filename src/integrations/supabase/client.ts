...
export const calculatePaidInvoicesTotalForSite = async (siteId: string): Promise<number> => {
  try {
    ...
    // Sum all net_amount values for invoices paid by supervisor
    ...
    return data.map(invoice => {
      try {
        ...
        return {
          ...
          paymentStatus: invoice.payment_status as PaymentStatus || PaymentStatus.PENDING,
          createdBy: invoice.created_by || '',
          createdAt: new Date(invoice.created_at || new Date()),
          siteId: invoice.site_id || '',
          status: invoice.payment_status as PaymentStatus || PaymentStatus.PENDING,
          vendorName: invoice.party_name || '',
          invoiceNumber: invoice.id.slice(0, 8),
