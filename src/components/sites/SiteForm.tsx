...
export default function SiteForm({ isOpen, onClose, onSubmit, supervisorId }: SiteFormProps) {
  ...
  // Proactive refresh on tab return
  useEffect(() => {
  ...
  const performSubmission = useCallback(async (values: SiteFormValues): Promise<boolean> => {
    ...
    try {
      ...
      const { data: insertData, error: insertError } = await withTimeout(doInsert(), SUBMIT_TIMEOUT_MS);
      if (insertError) throw insertError;
      if (!insertData || insertData.length === 0) {
        toast.error('No data returned from site creation.');
        return false;
      }
      return true;
